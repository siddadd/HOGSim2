...
for d=1:length(dataNames), dataName=dataNames{d};
  ...
  if(any(strcmp(dataName,{'InriaTest', 'InriaTrain', 'TudBrussels','ETH'})))
    bnds=[-inf -inf inf inf]; else bnds=bnds0; end
  if(strcmp(dataName,{'InriaTest', 'InriaTrain'}))
    i=find(strcmp({algs.name},'FeatSynth'));
    if(~isempty(i)), algs(i).resize=1; end;
  end
  if(strcmp(dataName, 'InriaTrain'))
      algs = algs_temp;
  end
  ...
  plotExps( res, plotRoc, plotAlg, plotNum, plotName, ...
    samples, lims, reshape([algs.color]',3,[])', {algs.style}, displayscoresonplot );
...
function plotExps( res, plotRoc, plotAlg, plotNum, plotName, ...
  samples, lims, colors, styles, displayscoresonplot)
...
for p=1:nPlots
  ...
  if( plotAlg )
    ...
    for d=1:nDt, 
        if (displayscoresonplot)
        lgd1{d}=sprintf('%2i%% %s',scores1(p,d),stra{d}); 
        else
        lgd1{d}=sprintf('%s',stra{d}); 
        end
    end
    %kp=[find(strcmp(stra,'VJ')) find(strcmp(stra,'HOG')) 1 1];
    % Changing this to ensure hw pedro and hw dalal are always plotted -
    % ska - 08/11/2015
    %kp=[find(strcmp(stra,'HWPedro')) find(strcmp(stra,'HWDalal')) 1 1];
    kp=[find(strcmp(stra,'OpenCV')) find(strcmp(stra,'MyOpenCV')) 1 1];
  ...
  if( plotRoc )
    yt=[0 .05 .1:.1:.5 .64 .8]; ytStr=sprintf('.%02i|',round(yt*100));
    set(gca,'XScale','log','YScale','log',...
      'YTick',[yt 1],'YTickLabel',[ytStr '1'],...
      'XMinorGrid','off','XMinorTic','off',...
      'YMinorGrid','off','YMinorTic','off');
    xlabel('false positives per image','FontSize',12);
    ylabel('miss rate','FontSize',12); axis(lims);
  else
  ...
  if isunix
    ...
    print('-painters', '-dpdf', '-r300', fName);
  end
end
